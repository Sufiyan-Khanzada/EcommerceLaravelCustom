   ...
   public function sendPayment()
   {
    ...
    //   die();
       if (!empty($token)) {
                ...
                $california_Tax = $this->session->userdata('california_tax')['cost'];
                ...
                if ($addrchk == 0) {
                    ...
                    $city = $customer->city;
                    $state = $this->db->get_where('states' , ['id' => $customer->state_id])->result()[0]->iso2 ?? '';
                    $country = $this->db->get_where('countries' , ['id' => $customer->country_id])->result()[0]->iso2 ?? '';
                ...
                if ($addrchk == 1) {
                    ...
                    $addr2 = $address2;
                    $zip = $postcode;
                    $state = $this->db->get_where('states' , ['id' => $state])->result()[0]->iso2 ?? '';
                    $country = $this->db->get_where('countries' , ['id' => $country])->result()[0]->iso2 ?? '';
                }
                ...
                $grand_total = $this->cart->total()+ $total_handling + $shipping_cost+$ctax;
                ...
                try {
                    ...
                    if ($res->getStatusCode() == 200) {
                        ...
                        $query = $this->db->get_where('orders', array('transaction_id' => $json->transaction_id));
                        if ($this->db->affected_rows() == 0) {
                            ...
                            if ($insert_products) {
                                ...
                                $this->db->where('payment_id', $json->transaction_id);
        ...
        $this->db->select('orderitems.*,products.title');
		$this->db->from('orderitems');
		$this->db->join('products', 'orderitems.product_id = products.product_id');
		$this->db->where('orderitems.order_id', $order_id);
		$orderitems = $this->db->get();
		$orderitems = $orderitems->result_array();

		$orders = $this->db->get_where('orders', array('order_id' => $order_id));
		$orders = $orders->result_array()[0];
		$customers = $this->db->get_where('customers', array('email' => $orders['customer_email']));
		$customers = $customers->result_array()[0];
		$states = $this->db->get_where('states', array('id' => $customers['state_id']));
		$states = $states->result_array()[0];
